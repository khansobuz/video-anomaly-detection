...
class CombinedLoss(nn.Module):
    def __init__(self, margin=20.0, alpha=0.8, beta=0.005, gamma=0.5):
...
def train(nloader, aloader, model, batch_size, optimizer, scheduler, warmup_epochs, device, epoch, accumulation_steps=2):
    model.train()
    optimizer.zero_grad()
    total_loss = 0
    for i in range(accumulation_steps):
        ...
        scores = scores.view((batch_size // accumulation_steps) * 32 * 2, -1).squeeze()
        abn_scores = scores[(batch_size // accumulation_steps) * 32:]
        nlabel = nlabel[:batch_size // accumulation_steps]
        alabel = alabel[:batch_size // accumulation_steps]
        loss_criterion = CombinedLoss(margin=20.0, alpha=0.8, beta=0.005, gamma=0.5)
...
if __name__ == '__main__':
    ...
    for epoch in range(args.max_epoch):
        result = train(loadern_iter, loadera_iter, model, args.batch_size, optimizer, scheduler, warmup_epochs, device, epoch, accumulation_steps=2)
        if result is None:
            continue
        ...
        if auc_res > best_auc:
            ...
            if patience_counter >= patience:
                ...
                break
        # Boost learning rate if AUC plateaus after 100 epochs
        if epoch >= 100 and auc_res < 0.95:
