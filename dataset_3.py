...
class Normal_Loader(Dataset):
    ...
    def __getitem__(self, idx):
        if self.is_train == 1:
            ...
            print(f"Normal_Loader train shape: {concat_npy.shape}")  # Debug
            ...
            concat_npy = np.concatenate([rgb_npy, flow_npy], axis=1)
            concat_npy = self.apply_augmentations(concat_npy)
            print(f"Normal_Loader test shape: {concat_npy.shape}")  # Debug
            if self.modality == 'RGB':
...
class Anomaly_Loader(Dataset):
    ...
    def __getitem__(self, idx):
        if self.is_train == 1:
            rgb_npy = np.load(os.path.join(self.path+'all_rgbs', self.data_list[idx][:-1]+'.npy'))
            flow_npy = np.load(os.path.join(self.path+'all_flows', self.data_list[idx][:-1]+'.npy'))
            concat_npy = np.concatenate([rgb_npy, flow_npy], axis=1)  # Fixed: Added axis=1
            concat_npy = self.apply_augmentations(concat_npy)
            print(f"Anomaly_Loader train shape: {concat_npy.shape}")  # Debug
            if self.modality == 'RGB':
                return rgb_npy
            elif self.modality == 'FLOW':
                return flow_npy
            ...
            print(f"Anomaly_Loader test shape: {concat_npy.shape}")  # Debug
            if self.modality == 'RGB':
